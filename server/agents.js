...
function spawnClaude(prompt) {
  return new Promise((resolve) => {
    const child = spawn('claude -p --model opus', { shell: true });
    let stdout = '';
    let stderr = '';
    let settled = false;
...
async function runAgentReal(id, prompt) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const { stdout, stderr } = await spawnClaude(prompt);
    if (stderr && stderr.trim()) {
      console.error(`[agent:${id}] stderr: ${stderr.trim()}`);
    }
    lastStdout = stdout || '';
    const parsed = extractJson(lastStdout);
    if (parsed) return normalizeResult(parsed);
  }
  return { bubble: '분석 실패', report: '(파싱 실패) ' + String(lastStdout).slice(0, 500) };
}
...
function mockResult(id, context = {}) {
  ...
  const table = {
    ...
    pm: {
      ...
      action: 'HOLD',
      confidence: 58,
      entry: `${sym} 트리거 확인 후에만 진입 (확인 전 진입 금지)`,
      stop: '무효화 레벨보다 한 단계 앞당긴 지점',
      target: '원안 목표 유지, 절반 도달 시 분할 익절',
    ...
    ace: {
      ...
      action: 'HOLD',
      confidence: 62,
      entry: `${sym} 20일선 상향 돌파 확인 시 분할 진입`,
      stop: '최근 20일 저점 이탈 시 손절',
      target: '직전 고점 부근을 1차 목표로 설정',
  ...
  if (context.mode === 'attack') {
    const attackTable = {
      ...
      ace: {
        ...
        action: 'BUY',
        confidence: 54,
        entry: `${sym} 직전 스윙 고점 돌파 시 진입`,
        stop: '직전 스윙 저점 이탈 시 손절',
        target: '상단 저항 부근을 1차 목표로 설정',
        rationale: `강세와 약세 논거가 균형에 가깝지만 단기 모멘텀이 근소하게 상방이라 롱을 택했습니다. 우위가 얇으므로 확신도는 낮게 두고, 무효화 레벨 이탈 시 즉시 청산을 전제로 합니다.`,
        scalp: {
          bias: 'LONG',
          entry: '직전 스윙 고점 돌파 확인 후 진입',
          stop: '직전 스윙 저점 이탈',
          target: '상단 저항 부근 1차 청산',
...
module.exports = { AGENTS, extractJson, runAgent, buildPrompt };
